...
.stat-card .lbl { font-size: 0.78rem; color: #64748b; margin-top: 0.3rem; text-transform: uppercase; letter-spacing: .06em; }
...
.event-card .etitle { font-size: 1.05rem; font-weight: 600; color: #f1f5f9; }
.event-card .etitle a { color: #60a5fa; text-decoration: none; }
.event-card .etitle a:hover { text-decoration: underline; }
.event-card .emeta  { font-size: 0.82rem; color: #64748b; margin-top: 0.3rem; }
.event-card .edesc  { font-size: 0.85rem; color: #94a3b8; margin-top: 0.5rem; line-height: 1.5; }
...
# ── Tabs ──────────────────────────────────────────────────────────────────────
tab_list, tab_timeline, tab_logs = st.tabs(["📋 Events", "📅 Timeline", "📊 Run Logs"])
    ...
    if not events:
        ...
        for ev in events:
            ...
            if start:
                ...
                if days >= 0:
                    pill = "days-urgent" if days <= 7 else ("days-soon" if days <= 30 else "days-pill")
                    days_html = f'<span class="{pill}">{days}d left</span>'

            meta_parts = [f"📅 {date_str}", src]
            if loc:     meta_parts.append(f"📍 {loc}")
            if country: meta_parts.append(f"🌏 {country}")
            ...
            with col_card:
                ...
                <div class="event-card">
                    <div class="etitle">
                        <span class="badge {badge_cls}">{icon} {cat}</span>
                        {days_html}
                        <br><a href="{url}" target="_blank">{title}</a>
                    </div>
                    <div class="emeta">{meta}{dead_str}</div>
                    {"<div class='edesc'>" + desc[:200] + "…</div>" if desc else ""}
                </div>
                """, unsafe_allow_html=True)
            with col_btn:
                st.markdown("<div style='margin-top:18px'>", unsafe_allow_html=True)
                bm_label = "⭐" if is_bm else "☆"
                if eid and st.button(bm_label, key=f"bm_{eid}", help="Save event", use_container_width=True):
                    toggle_bookmark(eid)
                    st.rerun()
